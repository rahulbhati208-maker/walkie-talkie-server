...
<head>
    ...
    <style>
        :root { --bg: #121212; --card: #1e1e1e; --border: #333; --accent: #2196f3; --danger: #f44336; --success: #4caf50; --text: #e0e0e0; --tab-height: 60px; }
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .bottom-nav { height: var(--tab-height); background: #1a1a1a; border-top: 1px solid var(--border); display: flex; justify-content: space-around; align-items: center; padding-bottom: env(safe-area-inset-bottom); flex-shrink: 0; z-index: 999; }
        ...
        .badge-dot { position: absolute; top: 10px; right: 35%; width: 10px; height: 10px; background: var(--danger); border-radius: 50%; border: 2px solid #1a1a1a; display: none; }
        ...
        .req-card { background: var(--card); border-radius: 8px; padding: 15px; display: flex; flex-direction: column; gap: 10px; border: 1px solid var(--border); min-height: 100%; }
        .code-input { background: #000; border: 1px solid #333; color: #0f0; padding: 10px; border-radius: 4px; font-family: monospace; font-size: 12px; width: 100%; }
...
<body>
    ...
    <script>
        ...
        function handleTraffic(type, data) {
            ...
            if (typeof displayBody === 'object') displayBody = new URLSearchParams(displayBody).toString();
        ...
        function forwardRequest() {
            ...
            if (currentPhase === 'request') {
                socket.emit('forward_request', { id: currentId, url: document.getElementById('int-url').value, method: document.getElementById('int-method').value, body: body });
            } else {
                socket.emit('forward_response', { id: currentId, status: document.getElementById('int-status').value, body: body });
            }
            clearInterceptUI();
        }
...
app.post('/api/repeat', async (req, res) => {
    try {
        ...
        const response = await axios({ url, method, data: body, headers: { 'User-Agent': 'Mozilla/5.0' }, validateStatus: () => true, responseType: 'arraybuffer' });
...
app.all('/proxy', async (req, res) => {
    ...
    try {
        io.emit('intercept_request', { id: reqId, url: targetUrl, method: incomingMethod, body: incomingBody });
