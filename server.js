...
app.post('/register', async (req, res) => {
    ...
    } catch (e) { res.status(500).send("Error: User may exist."); }
});
...
// --- CONTACT LIST (HIDE NUMBERS, SHOW LAST SEEN) ---
app.get('/my-contacts', async (req, res) => {
    const [contacts] = await pool.query(`
        SELECT u.id, u.profile_name, u.last_seen,
...
io.on('connection', (socket) => {
    ...
    socket.on('typing', (data) => io.to(`user_${data.toId}`).emit('is-typing', { fromId: userId }));
    socket.on('stop-typing', (data) => io.to(`user_${data.toId}`).emit('not-typing', { fromId: userId }));
